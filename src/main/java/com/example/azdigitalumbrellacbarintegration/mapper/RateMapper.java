...
public class RateMapper {
    ...
    public static List<RateEntity> mapValCursToListRateEntities(ValCurs valCurs) {
        LocalDate cbarDate = BuildHelper.buildDate(valCurs);
        ...
        for (Valute metal : metals) {
            RateEntity rate = BuildHelper.buildRateEntity(metal, Type.PRECIOUS_METAL, cbarDate);
            rates.add(rate);
        }
        for (Valute currency : currencies) {
            RateEntity rate = BuildHelper.buildRateEntity(currency, Type.FOREIGN_CURRENCY, cbarDate);
            rates.add(rate);
        }
        return rates;
    }
